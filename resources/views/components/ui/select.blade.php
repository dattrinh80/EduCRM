...
<div class="{{ $containerClass }}">
    ...
    <div class="relative w-full group">
        ...
        <select name="{{ $name }}" id="{{ $id }}" 
                {{ $attributes->merge(['class' => 'w-full ' . ($icon ? 'pl-10 ' : 'px-4 ') . 'pr-10 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all outline-none appearance-none cursor-pointer']) }}>
            {{ $slot }}
        </select>
