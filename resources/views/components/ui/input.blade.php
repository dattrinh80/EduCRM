...
<div class="{{ $containerClass }}">
    ...
    <div class="relative w-full group">
        ...
        <input type="{{ $type }}" name="{{ $name }}" id="{{ $id }}" placeholder="{{ $placeholder }}" value="{{ $value }}"
               {{ $attributes->merge(['class' => ($icon ? 'pl-10 ' : 'px-4 ') . 'pr-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all outline-none ' . ($uppercase ? 'uppercase tabular-nums' : '')]) }}>
    </div>
</div>
