...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-slate-500 mt-1">Define a new role and assign permissions</p>
        </div>
        ...
        <form action="{{ route('admin.roles.store') }}" method="POST" class="space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                ...
                <div class="space-y-1">
                    ...
                    <input type="text" name="name" id="name" required class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 text-sm transition" value="{{ old('name') }}" placeholder="e.g. Editor, Viewer, Supervisor...">
                    ...
                    <div class="border border-slate-100 rounded-xl overflow-hidden">
                        <div class="bg-slate-50 px-5 py-3 flex items-center justify-between">
                            ...
                            <label class="flex items-center gap-1.5 cursor-pointer text-xs text-slate-500">
                                <input type="checkbox" class="group-check w-3.5 h-3.5 rounded border-slate-300 text-primary-500 focus:ring-primary-500" data-group="{{ $group->id }}" onchange="toggleGroup(this, '{{ $group->id }}')">
                                All
                            </label>
                        </div>
                        <div class="px-5 py-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                            ...
                            <label class="flex items-start gap-2.5 cursor-pointer p-2.5 rounded-lg hover:bg-slate-50 transition">
                                <input type="checkbox" name="permissions[]" value="{{ $perm->id }}" class="perm-check perm-{{ $group->id }} w-4 h-4 rounded border-slate-300 text-primary-500 focus:ring-primary-500 mt-0.5" {{ in_array($perm->id, old('permissions', [])) ? 'checked' : '' }}>
                                <div>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        document.querySelectorAll('.perm-check').forEach(el => {
            el.addEventListener('change', () => {
                // Find group id from class list (perm-XXXX)
