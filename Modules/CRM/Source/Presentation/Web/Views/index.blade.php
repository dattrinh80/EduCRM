...
<div class="space-y-6" x-data="{ showCreateModal: {{ $errors->any() && !old('_method') ? 'true' : 'false' }} }">
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div class="p-4 border-b border-slate-100 bg-slate-50/50">
            <form action="{{ route('admin.sources.index') }}" method="GET" class="flex items-center gap-3">
                ...
                <div class="flex items-center gap-2">
                    ...
                    @if(!empty($search))
                    <a href="{{ route('admin.sources.index') }}" class="px-4 py-2 bg-slate-50 text-slate-600 hover:bg-slate-100 rounded-lg transition font-medium text-sm flex items-center gap-2 border border-slate-200 whitespace-nowrap">
