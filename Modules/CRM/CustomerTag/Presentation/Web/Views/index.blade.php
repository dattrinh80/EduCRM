...
@section('title', 'Quản lý Nhãn Customer')

@section('content')
<div class="space-y-6" x-data="customerTagManagementStore()">
    <!-- Header -->
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-display font-bold text-slate-800 tracking-tight">Nhãn KH</h1>
            <p class="text-slate-500 mt-1 flex items-center gap-1.5 text-sm">
                <i data-lucide="info" class="w-3.5 h-3.5 opacity-60"></i>
                Quản lý hệ thống nhãn màu sắc để phân loại khách hàng
            </p>
        </div>
        @can('leads.update')
        <x-ui.button variant="primary" icon="plus-circle" @click="loadModal('{{ route('admin.customer-tags.create') }}')">
            Thêm Nhãn
        </x-ui.button>
        @endcan
    </div>
    ...
    <x-ui.card bodyClass="p-0">
        ...
        <div class="p-4 border-b border-slate-100 bg-slate-50/50">
            <form action="{{ route('admin.customer-tags.index') }}" method="GET" class="flex items-center gap-3">
                ...
                <div class="flex items-center gap-2">
                    ...
                    @if(!empty($search))
                    <x-ui.button variant="ghost" icon="x-circle" :attributes="new \Illuminate\View\ComponentAttributeBag(['href' => route('admin.customer-tags.index'), 'tag' => 'a'])">
                        Xoá lọc
                    </x-ui.button>
                    @endif
                </div>
                        ...
                        <tr class="hover:bg-slate-50 transition group">
                            ...
                            <td class="p-4 px-6 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition">
                                    @can('leads.update')
                                    <button type="button" @click="loadModal('{{ route('admin.customer-tags.edit', $tag->id) }}')" class="p-1.5 text-slate-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition" title="Sửa">
                                        <i data-lucide="edit-2" class="w-4 h-4"></i>
                                    </button>
                                    @endcan
                                    @can('leads.delete')
                                    <form action="{{ route('admin.customer-tags.destroy', $tag->id) }}" method="POST" class="inline" onsubmit="return confirmDelete(this, '{{ addslashes($tag->name) }}')">
                                        @csrf
                                        @method('DELETE')
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('customerTagManagementStore', () => ({
