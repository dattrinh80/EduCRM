...
<form action="{{ route('admin.customer-tags.update', $tag->getId()) }}" method="POST" id="editCustomerTagForm" class="p-6 flex-1 overflow-y-auto">
    @csrf
    @method('PUT')
    ...
    <div class="space-y-4">
        <div class="space-y-1">
            ...
            <div class="relative">
                ...
                <input type="text" name="name" required class="w-full pl-10 pr-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 text-sm transition" value="{{ old('tag_id') == $tag->getId() ? old('name') : $tag->name }}">
            </div>
            @if(old('tag_id') == $tag->getId()) @error('name') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror @endif
        </div>

        <div class="space-y-1">
            <label class="text-sm font-medium text-slate-700 block">Màu sắc</label>
            <div class="grid grid-cols-5 gap-2">
                @php $currentColor = old('tag_id') == $tag->getId() ? old('color') : $tag->color; @endphp
                @foreach(['slate', 'red', 'orange', 'amber', 'yellow', 'lime', 'green', 'emerald', 'teal', 'cyan', 'sky', 'blue', 'indigo', 'violet', 'purple', 'fuchsia', 'pink', 'rose'] as $color)
                    <label class="relative flex items-center justify-center cursor-pointer group">
                        <input type="radio" name="color" value="{{ $color }}" class="peer sr-only" {{ $currentColor == $color ? 'checked' : '' }}>
            ...
            @if(old('tag_id') == $tag->getId()) @error('color') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror @endif
        </div>
    </div>
